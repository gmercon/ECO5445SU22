...
Mortgagestats <- Mortgagestats[c(5,6,9,11,13,20,24,30,34,39,40,59)]

# s6, s7, s13, s15, s17, s23a, s27a, s33, s40, s45, S46, school

Coln = c("Loan_Amount", "Action_Taken", "Race","Sex", "Income_Thousands", "Marital_Status", "Self_Employed", "Purchase_Price", "Credit_Approved", "DTI_Housing", "DTI_Total", "School")
...
Mortgagestats$NRace <- Mortgagestats$Race
Mortgagestats$NRace[Mortgagestats$NRace == 3] <- 0
Mortgagestats$NRace[Mortgagestats$NRace == 5] <- 1

Mortgagestats$NAT <- Mortgagestats$Action_Taken
Mortgagestats$NAT[Mortgagestats$NAT == 1] <- 1
Mortgagestats$NAT[Mortgagestats$NAT == 2] <- 1
Mortgagestats$NAT[Mortgagestats$NAT == 3] <- 0

Mortgagestats$NSex <- Mortgagestats$Sex
Mortgagestats$NSex[Mortgagestats$NSex == 1] <- 1
Mortgagestats$NSex[Mortgagestats$NSex == 2] <- 0
Mortgagestats$NSex[Mortgagestats$NSex == 3] <- mean(Mortgagestats$NSex)

Mortgagestats$NMS <- Mortgagestats$Marital_Status
Mortgagestats$NMS[Mortgagestats$NMS == "M"] <- 1
Mortgagestats$NMS[Mortgagestats$NMS == "U"] <- 0
Mortgagestats$NMS[Mortgagestats$NMS == "S"] <- 0


#Mortgagestats$NMCH <- Mortgagestats$Mortgage_Credit_History
#Mortgagestats$NMCH[Mortgagestats$NMCH == 1] <- 1
#Mortgagestats$NMCH[Mortgagestats$NMCH == 2] <- 0

Mortgagestats$NCA <- Mortgagestats$Credit_Approved
Mortgagestats$NCA[Mortgagestats$NCA == 0] <- 0
Mortgagestats$NCA[Mortgagestats$NCA == 1] <- 1
Mortgagestats$NCA[Mortgagestats$NCA == 666] <- 0
...
- Below we create our logistical regression model
...
- We see that our ROC best threshold is around .13 and our area under the curve is .7237
...
```
- Below we calculate several confusion matrices
...
logit.pred <- factor(prob > 0.13, levels = c(FALSE,TRUE),
                     labels = c("Approved","Denied"))
...
- After reviewing our models we see that logit.perf 2 has the highest accuracy with an ROC of 0.5. At our best threshold ROC level of .12 we see that our accuracy decreases, but our sensitivity increased drastically. This allows us to catch more denied mortgages while sacrificing some of the actual approved mortgages.
...
```{r}
Y <- as.matrix(Mortgagestats$NAT)
X1 <- as.matrix((Mortgagestats[-c(2,3,4,6,9,14)]))

nrow(Y) 
nrow(X1)

cv.reg1 <- cv.glm(X1, Y,
	family="binomial", verb=TRUE, standardize=TRUE, nfold = 10)
